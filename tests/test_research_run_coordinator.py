...
def _advance_to_awaiting_acceptance(ledger: RunLedger, coordinator: ResearchRunCoordinator) -> None:
    coordinator.transition("run-57", "alignment_projection_ready", "coordinator", expected_revision=ledger.get_revision("run-57"))
    ...
    coordinator.transition("run-57", "deliveries_compiled", "coordinator", expected_revision=ledger.get_revision("run-57"))
...
def test_completion_requires_all_canonical_obligations_and_is_terminally_idempotent(tmp_path) -> None:
    ...
    _append(
        ledger, "run-57", "closure-1", "slot-closure-assessment",
    ...
    _append(
        ledger, "run-57", "acceptance-1", "delivery-acceptance", {"decision": "accepted"},
    ...
    completed = coordinator.transition("run-57", "delivery_accepted", "human", expected_revision=ledger.get_revision("run-57"))
    ...
    assert any(item.id == "completion-record" and item.kind == COMPLETION_RECORD_KIND for item in ledger.load_run("run-57").artifacts)


def test_host_event_duplicate_is_idempotent_and_conflict_is_rejected(tmp_path) -> None:
    ledger, coordinator, _, _, _ = _initialize(tmp_path)
    event = coordinator.ingest_event(
        run_id="run-57", event_id="event-1", attempt_id="attempt-1", payload={"kind": "finding", "value": 1}, expected_revision=ledger.get_revision("run-57")
    )
    replay = coordinator.ingest_event(
        run_id="run-57", event_id="event-1", attempt_id="attempt-1", payload={"kind": "finding", "value": 1}, expected_revision=0
    )
    assert event == replay

    with pytest.raises(CoordinatorEventConflictError, match="event_id_conflict"):
        coordinator.ingest_event(
            run_id="run-57", event_id="event-1", attempt_id="attempt-1", payload={"kind": "finding", "value": 2}, expected_revision=0
        )
