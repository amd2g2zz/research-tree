...
def test_source_broker_is_the_only_capture_egress_service() -> None:
    ...
    for required in (
        ...
        "cpus: 0.5",
        "expose:\n      - \"8081\"",
        "source-capture-metadata:/var/lib/research-tree/source-captures",
        "source-broker",
        "source-egress",
    ):
    ...
    assert "source-capture-metadata:" in compose
...
def test_source_capture_metadata_is_evaluator_only_and_never_contains_content() -> None:
    compose = _read("compose.yaml")
    source_broker = _read("source_broker.py")

    assert 'SOURCE_CAPTURE_METADATA_DIR = Path("/var/lib/research-tree/source-captures")' in source_broker
    assert 'CAPTURE_METADATA_KEYS = ("captured_at", "final_url", "content_sha256", "status", "bytes")' in source_broker
    assert "SENSITIVE_QUERY_MARKERS" in source_broker
    assert "_metadata_url" in source_broker
    assert "os.replace" in source_broker
    assert "metadata_path.chmod(0o600)" in source_broker
    assert "source-capture-metadata" in compose
    runner = _service_block(compose, "runner")
...
def test_source_broker_persists_only_redacted_capture_metadata(tmp_path: Path) -> None:
    source_broker = _source_broker_module()
    ...
    metadata_files = tuple(tmp_path.glob("capture-*.json"))
    assert len(metadata_files) == 1
    payload = json.loads(metadata_files[0].read_text(encoding="utf-8"))
    assert set(payload) == {"captured_at", "final_url", "content_sha256", "status", "bytes"}
