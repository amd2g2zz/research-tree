...
def test_cli_emits_deterministic_real_registry_report(capsys: pytest.CaptureFixture[str]) -> None:
    ...
    assert payload["verified_groups"] == [1, 2, 3, 4, 5, 6, 7, 8, 9, 16, 32, 33, 34, 35]
    assert payload["unverified_groups"] == [group for group in range(6, 33) if group not in {6, 7, 8, 9, 16, 32}]
