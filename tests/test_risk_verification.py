...
def result_for(api_modules, request, *, status: str, failure=None, isolation=None):
    command_names = (
        ("spike",)
        if request.check_kind == "targeted_spike"
        else ("build", "hidden_acceptance", "regression")
    )
    return api_modules["IsolatedVerificationResult"](
        check_kind=request.check_kind,
        status=status,
        commands=tuple(
            {"name": name, "command": f"fake-{name}"} for name in command_names
        ),
        results=tuple(
            {
                "name": name,
...
def verify(api_modules, store, round_record, package, *, root: Path, tier: str, adapter=None):
...
def test_medium_spike_gets_a_sanitized_request_and_persists_evidence(tmp_path: Path) -> None:
    ...
    adapter = CapturingAdapter(
        lambda request: result_for(api_modules, request, status="pass")
    )
    ...
    assert evidence["executed_checks"][0]["commands"] == (
        {"name": "spike", "command": "fake-spike"},
    )
...
def test_risk_evidence_cannot_name_a_different_technical_package(tmp_path: Path) -> None:
    ...
    corrupted_payload["risk_verification"]["technical_package"]["ref"]["artifact_id"] = (
        "wrong-package"
    )
...
def test_high_risk_failure_is_classified_and_returns_same_round_follow_up(tmp_path: Path) -> None:
    ...
    diagnostic = next(
        ...
        if item["failure_category"] == "oracle_quality"
    )
