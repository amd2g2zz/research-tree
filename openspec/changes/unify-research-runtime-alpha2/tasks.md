...
- [ ] 4.2 Implement oracle result ingestion for method, environment, inputs, tool events, result artifacts, evaluator, verdict, and limitations. Implementation evidence: `OracleRun.from_mapping` validates exact result-artifact refs and `ResearchRunCoordinator.record_oracle_run` resolves current-run artifact revisions and content digests while preserving attempt/tool-event binding; `OracleAttempt` persistence remains open.
