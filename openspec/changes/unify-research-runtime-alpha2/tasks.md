...
- [x] 4.5 Make the core evaluator the only closure-token issuer and persist machine-readable pass, fail, and inconclusive explanations. Evidence: coordinator-owned token issuance, deterministic append-only revocation for material feedback, Blueprint Target rebinding, and direct Decision Ledger parent supersession; `tests/test_oracle_closure.py` covers reason-bearing revocation records/events and stale aggregate invalidation.
- [x] 4.6 Generate independent validation, method-switch, fallback, or residual-risk work after failed or inconclusive oracles. Evidence: `SQLiteRunLedger.schedule_oracle_successors` persists schema-valid WorkItems with exact OracleRun/Attempt/Spec inputs, uses method-switch vs independent-validation selection, protects run revisions, and is idempotent on replay; `tests/test_oracle_closure.py` covers persistence and replay.
...
- [x] 5.2 Implement coordinator initialization from an explicitly confirmed alignment artifact and exact Blueprint Target lineage. Evidence: canonical `run init` resolves schema-valid `AlignmentHandoff` and `BlueprintTarget` artifacts by exact revision/hash, verifies confirmation and Brief/Intent/Graph parent lineage before mutation, resumes committed initialization prefixes idempotently, and passes the group-5 contract and crash-prefix regressions.
- [x] 5.3 Implement transactional action dispatch, attempt leasing, Finding Pack ingestion, insight synthesis, decision convergence, readiness, and successor-work transitions. Evidence: coordinator-owned `stage_operations` transactions bind exact Blueprint Target, Work Item, Attempt Lease, Finding Pack, InsightDigest, DecisionLedgerEntry, ConvergenceRecord, P0 aggregate, and ReadinessRecord lineage; deterministic successor Work Items consume exact convergence/readiness deficits; focused fault injection and idempotency tests in `tests/test_alpha2_runtime_contract.py` plus shared contract tests cover every stage, and the full regression passes.
...
- [x] 5.5 Reject host, worker, hook, report-file, empty-frontier, and completed-wave completion attempts with persisted reasons. Evidence: the versioned `completion_claimed` HostEvent classifies all six proxy sources, `ResearchRunCoordinator.ingest_host_event` persists an idempotent `accepted=false` rejection with stable `completion_claim_rejected` reason and unmet obligations, and parameterized tests prove lifecycle state, revision, state digest, and obligations remain unchanged.
- [x] 5.6 Implement supersession and same-round replanning rules without mutating prior round artifacts. Evidence: semantic feedback classification routes target/priority/authority/success/task-identity changes through an atomic parent-linked successor run and routes depth/method/evidence corrections through `same_round_replan`; the lifecycle registry and runtime include explicit edges, prior completed snapshots remain immutable, dependent attempts/closures/obligations are invalidated, and correction integration plus dual-run fault-injection tests pass.
...
- [x] 8.3 Refactor the Codex adapter to dispatch native subagents and submit HostEvents without maintaining closure, readiness, report, or completion state. Evidence: `scripts/codex_execution_adapter.py` binds Codex to the stateless shared `host_event_adapter.py`; source and copied-package tests prove it emits a canonical HostEvent and creates no `.research-tree-native` state. The prior stateful runtime remains source-only legacy migration material and is not shipped in the Codex package.
- [x] 8.4 Refactor the Claude Code adapter independently for its package format and native tools while using the same HostEvent contract. Evidence: `scripts/claude_execution_adapter.py` independently binds `claude-code`, the generated Claude package contains the stateless translator rather than the legacy runtime, and source/copied-package parity tests validate its output.
...
- [x] 8.6 Add source-checkout, installed-package, Windows, POSIX, rebuild, and package-parity tests for both native adapters. Evidence: `tests/test_alpha2_native_event_emission.py` exercises source and copied-package entrypoints, `tests/test_skill_packages.py` enforces rebuild/parity, and GitHub Actions run `31060013316` passed the same focused suite on `ubuntu-latest` and `windows-latest`.
...
- [x] 8.8 Define activation evidence states for discovery, current installation, live body injection, and post-activation behavior; require explicit host receipts instead of inferring activation from a file read. Evidence: `skill_setup.activation_status` exposes four non-conflated states, advances live injection only from an exact native sentinel, leaves post-activation behavior unknown, and `tests/test_alpha2_activation_evidence.py` rejects file-read/package receipt inference.
...
- [x] 8.10 Add stale-link classification and non-destructive refresh handling to setup/status, including the legacy repository-root path failure mode. Evidence: setup reports `stale_link` without mutation, requires `--refresh-stale-link`, classifies a selected repository-root link as `legacy`, recognizes exact copy installs, and has focused link/status tests.
- [x] 8.11 Run native activation probes in isolated Codex, Claude Code, and Hermes fixtures; mark unavailable CLIs as unavailable evidence and retain exact outputs. Evidence: exact command/output tests cover all three hosts; real Codex and isolated project-scope Claude probes returned their exact host sentinels, while the missing Hermes CLI returned `host_cli_not_found`/`unavailable`. Windows `.CMD` execution and nonstandard `ComSpec` are covered, and raw stderr is retained only as length/digest metadata.

## 9. Hermes-Native Long-Horizon Adapter (#61)

- [x] 9.1 Map Hermes delegation, goals, Kanban tasks, and lifecycle outcomes to HostEvents without duplicating business state. Evidence: `scripts/hermes_execution_adapter.py` is stateless, projects canonical tasks, maps every Hermes lifecycle observation to HostEvent v1, and contains no checkpoint/report/completion path; `test_every_hermes_lifecycle_observation_maps_to_host_event` and state-absence tests cover the mapping.
- [x] 9.2 Project action evidence standards and closure oracles into Hermes goal acceptance criteria while retaining core verification authority. Evidence: deterministic task projections bind an exact Work Item artifact revision/hash, Attempt Lease dispatch digest, permission profile, Finding Pack schema, evidence classes, closure oracle, and retry policy; four Hermes success surfaces become rejected `completion_claimed` events in coordinator integration tests.
- [x] 9.3 Normalize provider/model identity, retry category, opaque error code, attempt identity, and safe gateway-log reference without persisting raw provider details. Evidence: shared wire and canonical validators accept only the five provider fields, bounded opaque codes, and `log:`/`sha256:` safe refs; focused tests reject raw fields and filesystem paths without echoing their values, while `hermes_skill_adapter.py doctor` retains category-only diagnostics.
- [x] 9.4 Implement retry, alternate provider, method switch, unknown-attempt recovery, and post-restart reconciliation within confirmed authority. Evidence: `plan-recovery` deterministically covers same-provider retry, allowed fallback provider, allowed method switch, stale/running/missing/unknown recovery, completed-output review, retry exhaustion, non-retryable failure, and authority blocking; restart integration creates a distinct canonical retry identity without human input.
- [x] 9.5 Treat hooks as fail-open observability and wake-up signals and test that hook failure cannot bypass evidence or completion gates. Evidence: the Hermes hook writes only a zero-byte `.research-tree/host-wakeups/hermes.signal`, ignores unrelated tools, returns `{}` on invalid input or an unwritable workspace, and never writes host business state; coordinator tests reject hook/goal/wave completion claims without changing revision, state digest, or lifecycle.
- [x] 9.6 Build Hermes-specific package and setup checks without copying Codex or Claude package structure. Evidence: the generated Hermes package contains its own execution, setup/diagnostic, runtime-hook, activation, and native-reference surfaces plus the shared wire translator; copied-package execution succeeds and tests prove Codex/Claude wrappers and references are absent. `build_skill_packages.py --check` validates all isolated packages.
- [x] 9.7 Run a long-horizon provider-failure fixture through restart and prove semantic parity with the native-host outcome. Evidence: `test_long_horizon_provider_restart_matches_native_host_semantics` executes provider exhaustion, canonical retryable state, process reconstruction, retry selection, HostEvent ingestion, and new lease creation for Hermes and Codex in separate workspaces, then matches canonical state, normalized attempt lineage, and semantic event digest.
