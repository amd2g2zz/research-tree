...
- [x] 1.1 Proposal, design (`impact_scope` + rejected designs), capability
  spec delta from issue #491 scenarios.
- [x] 1.2 GitNexus index rebuild + upstream impact per symbol (`plan`,
  `confirm`, `record` LOW; `_materialize` CRITICAL and excluded from the
  change surface).

## 2. Blocked disposition on the turn cap (RED first)

- [x] 2.1 RED: with an open high-impact requester-only gap, six user turns
...
- [x] 2.2 RED: with the ask budget spent on a high-impact requester-only gap
...
- [x] 2.3 RED: `record()` reports `next_action: "alignment_incomplete"` (not
  `reconnaissance`) when the dialogue is stalled on an exhausted high-impact
  gap.

## 3. Score gate and user decision paths (RED first)

- [x] 3.1 RED: a satisfied score allows the normal handoff path
...
- [x] 3.2 RED: an explicit `waive(reason)` records the waive, re-opens the
...
- [x] 3.3 RED: a user response after a blocked disposition re-opens the
...
- [x] 4.1 Score constants + `_alignment_score`; exit gate in `plan()` and
  `confirm()`; additive `alignment_score`/`alignment_exit_threshold` keys.
- [x] 4.2 `_blocked_disposition` + `_escalation_nodes`; turn-cap branch
  replaced; escalation branch added; `record()` `next_action` override.
- [x] 4.3 `waive()` store/CLI surface + `_active_waive` digest binding;
  `_extension_deadline` event-log derivation.
- [x] 4.4 Verify no existing test encodes the removed turn-cap escape; all
