...
1. `docs/adr/ADR-008-two-layer-contract.md`: the architecture ruling. Engine
   ...
   discipline #492, turn-shape #493, composition checks #499,
   structural-trace gates); prompt layer carries all open-ended behavior
   strategy. Includes the design test, the canonical contract-emission loop
   (emit terms → prompt layer composes freely → engine verifies traces →
   persist turn-record), and the rejected design (behavior enumeration in
   engine vocabulary: 13-action menus, fixed selection ladders).
2. `src/research_tree/turn_contract.py` (NEW, seam only): contract-terms
   schema (`target_gap`, `required_traces`, `cost_cap` with
   discrimination-vs-generation kinds, `taboos`), a frozen append-only
   trace-type registry seeded with the six initial types (option-set,
   concept-card, guess-statement, counterargument, possibility-survey,
   evidence-delta; duplicate registration rejected), and the
   `verify_traces()` primitive that fails naming the exact missing term —
...
3. `scripts/check_impact_scope.py` (NEW): deterministic, offline quality-gate
   helper that reconciles a GitNexus `detect-changes` JSON report (or, the
   CLI printing no machine-readable output, a `git diff --name-only
   <base>...HEAD` file-level cross-reference — documented limitation)
   against a declared `impact_scope` sidecar and FAILS on changed
   symbols/files outside the declared scope.
...
5. `.gitignore`: one line `plan.md` under "# Local configuration".
...
- Entirely additive: 2 new code-bearing files, 2 new docs/config files, 1 new
  openspec change folder, 1 `.gitignore` line. Zero existing symbols modified
  (GitNexus impact on the seam's new symbols: no upstream callers — it is
  deliberately uncalled until #489/#490 wire it; detect-changes
  reconciliation recorded in `evidence/` before push).
