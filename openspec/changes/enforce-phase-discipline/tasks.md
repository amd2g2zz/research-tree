...
- [x] 1.1 RED: every illegal phase transition (25 parametrized pairs) is
  rejected by `CanonicalResearchTreeStateService.transition` with a named
  error; legal edges and self-loops pass.
- [x] 1.2 RED: unknown `phase` values, non-`compiled` birth phases, and
...
- [x] 2.1 RED: a `strategy_authority_fingerprint` change without a
...
- [x] 3.1 `tree_state.py`: `TREE_PHASES`, `DEFAULT_TREE_PHASE`,
  ...
  realignment gate.
- [x] 3.2 GREEN: sections 1-2 pass; existing recursive-search and
  handoff suites stay green.

## 4. Two-option re-entry protocol (RED first)

- [x] 4.1 RED: `resolve_research_reentry` maps reopen / supplemental /
  status prompts to their named paths and refuses drift and bare
  interruptions (`research_reentry_refused`).
- [x] 4.2 RED: with run phase `research`, `observe` records the re-entry
...
- [x] 5.1 `lifecycle_hook.py`: phase source chain
...
- [x] 5.2 GREEN: section 4 passes; existing lifecycle-hook suites stay green.

## 6. Prompt layer

- [x] 6.1 Replace the internal-successor-revision clause in
...
- [x] 6.2 Regenerate `packages/**` with
  `uv run --frozen python scripts/build_skill_packages.py` and commit the
  regenerated artifacts.

## 7. Gate

- [x] 7.1 Full local gates green (pytest, ruff check + format,
